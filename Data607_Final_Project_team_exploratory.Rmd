...
author: "Mohamed Hassan-El Serafi & Eddie Xu"
date: "2023-04-29"
...
tc_data_21 <- tc_data_21 %>%
  data.frame(check.names =  FALSE) %>%
  rename_with(~ gsub("LeagueDashTeamClutch.", "",.x, fixed=TRUE)) %>%
  add_column(team_season = "2021-21")
...
path_csv2 <- getwd()
path_csv3 <- getwd()
write.csv(nba_all_clutch_data, file.path(path_csv, "Team_Clutch_Data.csv"))
write.csv(nba_all_playerclutch_data, file.path(path_csv2, "Player_Clutch_Data.csv"))
write.csv(historical_team_stat, file.path(path_csv3, "Team_Stat_Regular_Season_Datat.csv"))
```
